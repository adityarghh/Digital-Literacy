# Task 2 – Student Digital Portfolio

## Platforms Set Up

| Platform | Purpose | Profile Link |
|----------|---------|--------------|
| GitHub | Code hosting and project portfolio | [github.com/adityarghh](https://github.com/adityarghh) |
| LinkedIn | Professional networking and internships | www.linkedin.com/in/aditya-raj-79a53b314 |
| Kaggle | Data science and ML practice | https://www.kaggle.com/adityarghh |

## GitHub Profile README
Created a profile README repository with:
- Name and branch (B.Tech AI, VIT Bhopal)
- Current year of study
- Learning goals and interests

## LinkedIn
Profile includes:
- B.Tech Artificial Intelligence – VIT Bhopal University (2025–2029)
- Clubs: Bit By Bit (Core Member), PranSpanda (Design & Content)
- Certifications: Deloitte Data Analytics, Deloitte Cyber Security

## Kaggle
Created profile for data science and ML practice — relevant to AI/ML specialisation.

## Files
- README.md – this file
- github-profile.png – screenshot of GitHub profile
- linkedin-profile.png – screenshot of LinkedIn profile
- kaggle-profile.png – screenshot of Kaggle profile
