# 📚 CSE0001 – Digital Literacy Portfolio

**VIT Bhopal University | VITyarthi E-Learning Platform**

---

## 👤 Student Details

| Field | Details |
|-------|---------|
| **Name** | Aditya Raj |
| **Registration No.** | 25BAI10708 |
| **Branch** | B.Tech Artificial Intelligence |
| **Year** | First Year B.Tech (2025–2029) |
| **Course Code** | CSE0001 |

---

## 📝 Module Summaries

### Task 1 – Digital Literacy Infographic (Module 1)
Created a one-page infographic using **Canva** covering digital literacy, safe internet practices, email etiquette, and useful digital tools for students.

### Task 2 – Student Digital Portfolio (Module 2)
Set up accounts on **GitHub**, **LinkedIn**, and **Kaggle**. Created a GitHub profile README and completed the LinkedIn Education section.

### Task 3 – Coding & Collaboration Platforms (Module 3)
Completed **'Say Hello, World! With Python'** on HackerRank and built a 5-question **Digital Literacy Awareness Quiz** on Google Forms.

> 🔗 **Digital Literacy Quiz Link:** [paste your Google Form link here]

### Task 4 – Email Etiquette (Module 4)
Drafted two professional emails and created a Social Media Do's & Don'ts checklist. Files in `task-4-email-etiquette/`.

### Task 5 – Cybercrime Awareness (Module 5)
Wrote a UPI fraud case study and a 10-point prevention checklist including **cybercrime.gov.in** and helpline **1930**.

---

## 🔗 Quick Links

- 📄 [Project Report](./report/)
- 🎨 [Task 1 – Infographic](./task-1-presentation/)
- 👤 [Task 2 – Portfolio](./task-2-portfolio/)
- 💻 [Task 3 – Platforms](./task-3-platforms/)
- 📧 [Task 4 – Email Etiquette](./task-4-email-etiquette/)
- 🔒 [Task 5 – Cybercrime](./task-5-cybercrime/)
